...
import { collection, getDocs, doc, updateDoc, getDoc, setDoc, addDoc,where, collectionGroup} from 'firebase/firestore/lite';
 ...
 const getdata = async()=>{
   const q= collection( db, 'Trader');
    ...
    const data = snapShot.docs.map((doc) => ({  
     ...
     data.map(async(elem)=>{
      const workq= collection( db, "Trader/"+user_id+"/MyBookings");
    const mybooking = await getDocs(workq);   
    const mybookinginfo = mybooking.docs.map((doc) => ({  
      ...doc.data(),
      id: doc.id
    }))
     setDetails(mybookinginfo);
     })
}
     ...
     <FlatList
       ...
       renderItem = {({ item }) => 
         ...
         <View style={{
           ...
           <Image source={{uri: item.Product_img}} style={{height: 120, width: 150, borderRadius: 10, borderTopLeftRadius: 0, borderBottomLeftRadius: 0 }} />
           <Text style={{ fontWeight: 'bold', marginLeft: 160, fontSize: 26, marginTop: -120,  }}> { item.FarmerName}</Text>
           <Text style={{
             paddingLeft: 163,
             marginTop: 5,
             marginLeft: 2,
 ...
 useEffect(()=>{
  ...
  const updateTask = async(Quan,quan, id, inputU) =>{
    ...
    if(Quan == 0 || Quan <=20){
      ...
      //const col = collection(db, "TraderMyBookings");
      const col = collection(db, "Trader/"+user_id+"/MyBookings")
     const payLoad = {Phno: phno, FarmerName: farmerName, Product_img: img, traderID: user_id};
